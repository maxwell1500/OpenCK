...
void NpcValidator::validate(const Data& data, Messages& messages)
{
    ...
    for (int i = 0; i < npcCollection.size(); ++i)
    {
        ...
        else if (raceCollection.searchId(npc.editorId) == NOT_FOUND && raceCollection.size() > 0)
        {
