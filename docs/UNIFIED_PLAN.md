...
| 24.9 | Object Window keyword filter UI (user-created filters, saved) | ✅ Object Window has a saved-filter dropdown + Save/Delete buttons next to the filter box; named filters persisted via QSettings (`SavedFilter/<name>/text`), applied as text filters. `.filter` file import/export remains deferred (15.3) |
...
| 19 — Material Editor & Asset Pipeline | 3/7 | ◐ |
...
Phase 19: Material editor & asset pipeline (DDS import/decode, rule templates, texture conversion rules done; BSMaterial graph, mesh/phys LOD, FBX→NIF pending)
