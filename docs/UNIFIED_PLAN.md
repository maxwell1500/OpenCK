...
| 23.4 | Real plugin compaction: form-ID renumbering + reference re-pointing (not count-and-save) | ? \PluginCompactor\ (\src/model/tools/plugincompactor.hpp/.cpp\): \collectFormIds\ (dedup from all collections), \uildMap\ (dense local-ID renumbering preserving the master-index byte), \emap\ (unmapped/master-owned IDs pass through), \epointRefr\ (base/owner/script refs) + \epointCell\ (owner); \IRecordCollection::setFormId\ added (promotes base records to Modified so new IDs persist on save); Compact Small Master action now renumbers every record's form ID and re-points REFR/CELL references before saving; \	est_plugincompactor\. NOTE: ESL (light master) authoring added - \ESMWriter::setFileFlags\ preserves the TES4 header flags (Master/LightMaster) on save (previously always 0), \Document\ carries the loaded flags and can mark a new file light master; \FormIdCompactor\ (\src/model/tools/formidcompactor.hpp/.cpp\) remaps a plugin's owned records into the ESL 0x000-0xFFF range with a 4096-record ceiling, rewriting the typed FormID reference fields of RELA/ECZN/IPDS/IPCT/HAZD/SHOU; \	est_esl\. References embedded in opaque raw subrecords remain a follow-up. |
