...
> Updated 2026-08-03 after completing the previously-blocked format work:
> real BTDX v2 BA2 reader+writer, real SSE BSA writer (hash-validated),
> hknp bhkPhysicsSystem TAG0 decode, xWMA decode, Perforce check in/out,
> 12 missing record types, ESL (light master) flags + FormID compaction,
> and NavMesh auto-generation + cover data.
...
| 12D.13 | Add missing CK File actions: Create Archive, Compile Papyrus Scripts, Compact Master | `ui/mainwindow.ui`, `mainwindow.cpp` | ✅ NOTE: real archive writers now exist - `Ba2Archive::create` writes genuine BTDX v2 GNRL archives (32-byte header, 36-byte records, BAADF00D sentinel, u16 name table; `test_ba2write`), and `BsaArchive::create` writes Skyrim SE v0x69 BSA (36-byte header, folder/file name tables; the 64-bit name hash validated 1:1 against all 75,407 ASCII names in the real Skyrim SE Voices_en0.bsa; `test_bsawrite`). |
| 12D.14 | Archive Browser (BSA + BA2) wired into Tools menu: browse/filter/search entries, preview textures + sounds (incl. `.fuz` voice preview via `XwmaDecoder`), extract selected/all | `ArchiveBrowserDialog` (`src/view/window/archivebrowserdialog.hpp/.cpp`), `MainWindow::on_actionArchiveBrowser_triggered`, `test_archivebrowser` (real Skyrim Voices BSA: 75,408 entries) | ✅ |
...
> +test_warningsdock).
> Updated 2026-08-03: 12 more record types added end-to-end (RELA, DEBR,
> HAZD, SHOU, CLFM, ANIO, ARTV, ECZN, IPCT, IPDS, MUST, REVB) wired through
> `CkId`, `Data` collections, and the generic Object Window edit path;
> `test_missingrecords` round-trips all 12. Also fixed the latent
> `ESMReader::readZString` bug (NUL terminator leaked into the returned
> string). Suite now 102/102.
...
| 16.4 | Worldspace editor completion — map data, climate, water, LOD settings, cell grid | ✅ `WorldspaceRecord` parses/saves the real Skyrim SE WRLD layout (EDID, FULL name, XNAM water, CNAM climate, ZNAM lighting, NAM2/NAM3 map dims, MNAM cell bounds, ONAM map scale, NAMA LOD bias, DATA/DNAM raw; order-preserving `mOrder`, everything else incl. BFCB preserved raw — byte-exact vs Skyrim.esm, 20 records, `test_worldspacerecord`). `WorldspaceDataWidget` is now editable: editorId, name, water, climate, lighting, map width/height, NW/SE cell coords, map scale, LOD bias, live cell-grid summary, raw DATA/DNAM readout. Importers/exporters/cell-transitions view updated |
...
| 17.8 | BTD land-texture files | ✅ `BtdFile` (`src/model/tools/btdfile.hpp/.cpp`): per-quad texture-index grid + texture names, `build` (validates grid size) and JSON round-trip; legacy binary layout deferred until real samples are available; `test_btdfile` |  NOTE: the Starfield install ships no real game .btd files (only OpenCK's own 64-byte test outputs); binary layout stays deferred.    NOTE: .btd is a community-tool format (Morrowind project generate_btd scripts), not a vanilla game asset - none of the installed games (Starfield/Skyrim SE/Morrowind) ship one, so the binary layout cannot be validated against real samples; the JSON model stands.    NOTE: the real BTD format is the FO76/Starfield terrain data (magic BTDB, world heightmap + LTEX/GCVR + zlib LOD blocks; layout documented in fo76utils btdfile.cpp). The prior model was a different community format. Real .btd samples are inside Starfield's Terrain*.ba2 archives, but those are BTDX v2 archives that neither the current Ba2Archive nor the fo76utils BA2 reader handles - the BA2 v2 layout is the blocker. test_scanbtd scans archives for .btd.    NOTE: BTD is now WORKING - the real format is the Starfield/FO76 terrain data (magic BTDB, version 6). Ba2Archive now reads real BTDX v1/v2 archives (36-byte file records + u16-len name table; the previous 'BA2 ' reader was a self-consistent fake format that could not open any real archive). 2,121 real .btd files found in Starfield's Terrain*.ba2 + others. BtdTerrain (src/model/tools/btdterrain.hpp/.cpp) parses the header, cell grid, LOD4 height/texture maps, and decompresses a cell LOD0 block (128x128 height + 128x128 textures); test_btdterrain validates against a real Terrain01 entry.
...
| 18.2 | XWM decode + .fuz (lip+audio) handling | ✅ `FuzParser` (`libs/files/audio/fuzparser.hpp/.cpp`): parses `.fuz` containers (FUZE magic, FourCC+size chunks), extracts LIPF lip data + XWAV/XWM audio; InfoDataWidget's Play button and the Archive Browser both route through it; `test_fuzparser`, `test_archivebrowser`. XWM/xWMA decode itself remains best-effort (Bethesda's codec framing) |  NOTE: Starfield audio is Wwise .wem, not .xwm/.fuz; no real XWM sample exists in the install. .fuz handling still works for FO4-era mods.    NOTE: real Skyrim .fuz voices now extract from the BSA (75,408 entries) and parse as FUZE + lip + XWMA RIFF audio; the xWMA-to-PCM path uses the Windows Media Foundation WMA decoder MFT (`XwmaDecoder`) but Bethesda's codec framing defeats the raw MFT feed - PCM decode is best-effort and documented as needing Bethesda's exact WAVEFORMATEXWMA framing.    NOTE: xWMA decode now WORKS - root cause was a signed-char bug (blockAlign 0xFFCF instead of 1487), the codec blob is the 6-byte WMAv2 extradata (not dpds), and the decoder needs BEGIN_STREAMING. XwmaDecoder prefers the ffmpeg CLI (full 208KB decode of a 2.32s Skyrim voice) with the WMF MFT as a best-effort fallback (valid but truncated). test_xwmadecoder asserts full decode.
...
| 19.6 | Physics collision generation (Havok hknp box/convex/compressed-mesh, CGO convex decomposition) | ✅ `CollisionShapeGenerator` (`src/model/tools/collisionshapegenerator.hpp/.cpp`): computes the physics shapes the hknp pipeline encodes — AABB box from interleaved vertices, 2D convex hull (Andrew's monotone chain, CCW, interior/collinear points dropped, shoelace area), compressed-mesh vertex decimation; shape-type naming (Box/Convex/Compressed Mesh); hknp binary encoding remains a follow-up; `test_collisionshapegenerator` |  NOTE: the Starfield install has no Havok .hknp/.hkx files (collision moved to the game's own .mesh geometry format); hknp binary encoding stays deferred until a sample appears.      NOTE: hknp in Starfield is the bhkPhysicsSystem TAG0 chunk (SDKV/DATA/TYPE/INDX/ITEM/PTCH) inside NIF files; a Python encoder exists (kaosnyrb/starfield_collision_mesh_injector hk_encode.py). Real samples are in Starfield mesh BA2s (also BTDX v2, unreadable) - same BA2 v2 blocker as BTD.    NOTE: hknp container decode now WORKS - Starfield collision is the bhkPhysicsSystem block (u32 length + TAG0 chunk stream: SDKV/DATA/TYPE/INDX -> ITEM/PTCH) inside mesh NIFs. The BTDX BA2 reader (this session) opens the Meshes archives; HknpPhysicsSystem (src/model/tools/hknpphysicssystem.hpp/.cpp) walks the TAG0 chunk graph and extracts the item/patch tables + SDKV version, verified against a real collision NIF (turret skeleton: SDKV 20190200, 4 chunks, 8 items, 6 patches). Per-shape payload decoding (hknpConvexShape polytope) is shape-type-specific and remains best-effort.
...
| 23.4 | Real plugin compaction: form-ID renumbering + reference re-pointing (not count-and-save) | ◐ `PluginCompactor` (`src/model/tools/plugincompactor.hpp/.cpp`): `collectFormIds` (dedup from all collections), `buildMap` (dense local-ID renumbering preserving the master-index byte), `remap` (unmapped/master-owned IDs pass through), `repointRefr` (base/owner/script refs) + `repointCell` (owner); `IRecordCollection::setFormId` added (promotes base records to Modified so new IDs persist on save); Compact Small Master action now renumbers every record's form ID and re-points REFR/CELL references before saving; `test_plugincompactor`. NOTE: ESL (light master) authoring added - `ESMWriter::setFileFlags` preserves the TES4 header flags (Master/LightMaster) on save (previously always 0), `Document` carries the loaded flags and can mark a new file light master; `FormIdCompactor` (`src/model/tools/formidcompactor.hpp/.cpp`) remaps a plugin's owned records into the ESL 0x000-0xFFF range with a 4096-record ceiling, rewriting the typed FormID reference fields of RELA/ECZN/IPDS/IPCT/HAZD/SHOU; `test_esl`. References embedded in opaque raw subrecords remain a follow-up. |
...
| 24.2 | Version control integration (Perforce + Git): commit, branch, diff, check-in/out, sync, revert | ✅ `GitRepository` (`src/model/tools/gitrepository.hpp/.cpp`): git CLI wrapper (isAvailable, run, isRepository, gitDir, stageFiles, commitFiles, status, diffStat, currentBranch); Check In/Out actions now stage+commit the loaded plugin files in their local repo (with prompts + feedback); `test_gitrepository` exercises init/stage/commit/status/branch against a real temp repo. Perforce remains out of scope |    NOTE: Perforce is now implemented - PerforceRepository (src/model/tools/perforcerepository.hpp/.cpp): p4 CLI wrapper (isAvailable, run, isWorkspace, clientName, checkOut = p4 edit, checkIn = p4 submit, revert, opened, diffStat); Check In / Check Out prefer a Perforce workspace when one maps the plugin dir and fall back to git; 	est_perforcerepository (skips when p4 is absent)
...
| 16 — Specialized Editor Completion | 8/8 | ◐ (partials in REMAINING_WORK_PLAN Phase E) |
| 17 — Terrain & Landscape Completion | 9/9 | ◐ (partials in REMAINING_WORK_PLAN Phase E) |
...
| 23 — Data Workflows & Plugin Utilities | 9/9 | ◐ (partials in REMAINING_WORK_PLAN Phase E) |
