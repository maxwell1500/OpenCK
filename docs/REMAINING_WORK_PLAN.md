...
| E4 | **M8** SCEN PHDA binary | ✅ Done (resolved 2026-08-12 as a finding): real-data audit across all three shipped games with SCEN — Skyrim.esm (1706 SCEN), Starfield.esm (7613 SCEN), and Fallout4.esm (3568 SCEN) — contains **zero PHDA subrecords**. All three use the new-generation scene schema (FNAM/HNAM/NAM0/NEXT/WNAM/ALID/LNAM/DNAM/ANAM/INAM/PTOP/NTOP/…). PHDA is therefore not a format any shipped game's SCEN records exercise; shipping a "validated" PHDA encoder would be validating against no real data. Resolution: keep the byte-exact raw-subrecord round-trip (already `test_scenrecord`-covered) and document PHDA as a classic-format vestige with no live payloads. The `test_dumpesm` diagnostic's path/dump-count parameters were used for this audit. | round-trip test vs real data |
...
| SCEN PHDA encoder (E4) has no real-data sample | Cannot validate encoder | ✅ Resolved as a finding — Skyrim.esm (1706 SCEN), Starfield.esm (7613 SCEN), Fallout4.esm (3568 SCEN) all contain zero PHDA; PHDA is a classic-format vestige with no live payloads. Raw round-trip stays byte-exact; no encoder shipped for a format no real file uses. |
