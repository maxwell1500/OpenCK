...
QString DataExporter::getEditorId(const BaseRecord& record)
{
    ...
    if (auto* r = dynamic_cast<const Record<RefrRecord>*>(&record)) return r->get().editorId;
...
QStringList DataExporter::recordToCSVFields(const BaseRecord& record, CkId::Type type, QStringList& headers)
{
    ...
    case CkId::Type_Class_: {
        const auto& c = dynamic_cast<const Record<ClassRecord>&>(record).get();
        if (headers.size() == 3) {
            headers << "Flags" << "ClassName" << "Description" << "Faction" << "IconPath";
        }
        fields << QString::number(c.flags) << c.className << c.description
               << QString::number(c.faction) << c.iconPath;
        break;
    }
    case CkId::Type_Fact_: {
        const auto& f = dynamic_cast<const Record<FactRecord>&>(record).get();
    ...
    case CkId::Type_WRLD_: {
        const auto& w = dynamic_cast<const Record<WorldspaceRecord>&>(record).get();
        if (headers.size() == 3) {
            headers << "Flags" << "WaterType" << "ClimateId" << "LightingId" << "MapSize"
                    << "CellIds" << "NavPointIds";
        }
        fields << QString::number(w.flags)
    ...
    case CkId::Type_Material_: {
        const auto& m = dynamic_cast<const Record<MaterialRecord>&>(record).get();
        if (headers.size() == 3) {
            headers << "Flags" << "Name" << "Description" << "IconPath" << "ModelPath"
                    << "TexturePath" << "MaterialType" << "Value" << "Weight" << "Health"
                    << "Magicka" << "Stamina" << "Level" << "Race" << "Faction" << "Stage" << "Difficulty";
        }
        fields << QString::number(m.flags) << m.name << m.description
               << m.iconPath << m.modelPath << m.texturePath << m.materialType
               << QString::number(m.value) << QString::number(m.weight)
               << QString::number(m.health) << QString::number(m.magicka)
               << QString::number(m.stamina) << QString::number(m.level)
               << QString::number(m.race) << QString::number(m.faction)
               << QString::number(m.stage) << QString::number(m.difficulty);
        break;
    }
    case CkId::Type_Land_: {
        const auto& l = dynamic_cast<const Record<LandRecord>&>(record).get();
...
QDomElement DataExporter::recordToXML(const BaseRecord& record, CkId::Type type, QDomDocument& doc)
{
    ...
    case CkId::Type_Material_: {
        ...
        addStr("materialType", m.materialType);
        addInt("value", m.value);
